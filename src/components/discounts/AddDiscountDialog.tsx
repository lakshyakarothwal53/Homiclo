...
export function AddDiscountDialog({
  ...
  triggerLabel?: string;
  title: string;
  /** When set, the value type is fixed (Flat / Percentage pages). */
  lockType?: DiscountValueType;
  /** When set, the applies-to type is fixed (Product / Category pages). */
  lockTarget?: DiscountTargetType;
  /** Existing row when editing — prefills the form. */
  initial?: PromoRow;
  /** Custom trigger element (e.g. an "Edit" link); defaults to the Add button. */
  trigger?: ReactNode;
  onSubmit: (row: PromoRow) => void;
}) {
  ...
  const [appliesToType, setAppliesToType] = useState<DiscountTargetType>(lockTarget ?? "product");
  const [targets, setTargets] = useState<DiscountTarget[]>([]);

  function reset() {
    setName(initial?.name ?? "");
    setCode(initial?.code ?? "");
    setValueType(initial?.valueType ?? lockType ?? "percentage");
    setValue(initial ? String(initial.value) : "");
    setMinOrder(initial ? String(initial.minOrder) : "");
    setValidFrom(initial?.validFrom ?? "");
    setValidTo(initial?.validTo ?? "");
    setCap(initial?.cap != null ? String(initial.cap) : "");
    setAppliesToType(initial?.appliesToType ?? lockTarget ?? "product");
    setTargets(initial?.appliesTo ?? []);
  }

  // Re-sync fields whenever the dialog opens (e.g. editing a different row).
  useEffect(() => {
    if (open) reset();

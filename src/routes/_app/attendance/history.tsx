...
import { useEmployeeAttendance, useSelfAttendanceSummary } from "@/hooks/use-attendance";
...
function Page() {
  ...
  const teamQuery = useEmployeeAttendance(search, { from: period.from, to: period.to }, homeBranch);
  const selfQuery = useSelfAttendanceSummary(employeeId, { from: period.from, to: period.to });
  const { data: employees = [], isLoading, refetch } = selfOnly ? selfQuery : teamQuery;
  ...
  return (
    <>
      ...
      {/* Branch-wide roll-up is supervisory: not rendered at all for the
          employee role, which only ever sees its own row below. */}
      {!selfOnly && (
      ...
      <Card className="border-border">
        <CardHeader>
          <CardTitle className="text-base">
            {selfOnly ? "My Attendance Record" : "Attendance Records"}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            {/* Search is a roster control — an employee only ever has one row. */}
            {!selfOnly && (
              <Input
